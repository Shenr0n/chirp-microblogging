import React, { useEffect, useState } from 'react'
import KeyboardBackspaceIcon from '@mui/icons-material/KeyboardBackspace';
import { useNavigate, useParams } from 'react-router-dom';
import { Avatar, Box, Button, Tab, Tabs } from '@mui/material';
import OpenInNewIcon from '@mui/icons-material/OpenInNew';
import LocationOnIcon from '@mui/icons-material/LocationOn';
import CalendarMonthIcon from '@mui/icons-material/CalendarMonth';
import { TabContext, TabList, TabPanel } from '@mui/lab';
import ChirpCard from '../homeSection/ChirpCard';
import ProfileModal from './ProfileModal';
import { useDispatch, useSelector } from 'react-redux';
import { findUserById, followUserAction } from '../../store/auth/Action';
import { findChirpsByLikeContainsUser, getUsersChirp } from '../../store/Chirp/Action';

const Profile = () => {
    const [tabValue, setTabValue] = useState("1")
    const navigate = useNavigate();

    const [openProfileModal, setOpenProfileModal] = useState(false);
    const handleOpenProfileModal = () => setOpenProfileModal(true);
    const handleClose = () => setOpenProfileModal(false);
    const { auth, chirp } = useSelector(store => store);
    const dispatch = useDispatch();
    const { id } = useParams();


    const handleBack = () => navigate(-1);

    const handleFollowUser = () => {
        dispatch(followUserAction(id))
        console.log("follow user")
    }

    const handleTabChange = (event, newValue) => {
        setTabValue(newValue)
        if (newValue === 4) {
            console.log("Likes chirp")
        }
        else if (newValue === 1) {
            console.log("Users chirps")
        }
    };

    useEffect(() => {
        dispatch(findUserById(id));
        dispatch(getUsersChirp(id));

        dispatch(findChirpsByLikeContainsUser(id));
        console.log("Found liked chirps");

    }, [id]);

    return (
        <div>
            <section className={`bg-white z-50 flex items-center sticky top-0 bg-opacity-95`}>

                <KeyboardBackspaceIcon className='cursor-pointer' onClick={handleBack} />
                <h1 className='py-5 text-xl font-bold opacity-90 ml-5'>
                    {auth.findUser?.fullName}'s Profile
                </h1>
            </section>

            <section>
                <img className='w-[100%] h-[15rem] object-cover'
                    src={auth.findUser?.backgroundImage || "https://cdn.pixabay.com/photo/2018/10/05/10/56/landscape-3725657_1280.jpg"} />
            </section>

            <section className='pl-6'>
                <div className='flex justify-between items-start mt-5 h-[5rem]'>
                    <Avatar
                        className='transform -translate-y-24'
                        alt='profile picture'
                        src={auth.findUser?.image || "https://upload.wikimedia.org/wikipedia/commons/2/2c/Default_pfp.svg"}
                        sx={{ width: "10rem", height: "10rem", border: "4px solid white" }} />

                    {auth.findUser?.req_user ?
                        (<Button
                            onClick={handleOpenProfileModal}
                            variant='contained' sx={{ borderRadius: "20px" }}>Edit Profile
                        </Button>
                        ) : (
                            <Button
                                onClick={handleFollowUser}
                                variant='contained' sx={{ borderRadius: "20px" }}>
                                {auth.findUser?.followed ? "UnFollow" : "Follow"}
                            </Button>
                        )}
                </div>

                <div>
                    <div className='flex items-center'>
                        <h1 className='font-bold text-lg'>{auth.findUser?.fullName}</h1>
                        {true && (
                            <img
                                className='m1-2 w-5 h-5'
                                src="https://cdn-icons-png.flaticon.com/512/7641/7641727.png"
                                alt="" />)}

                    </div>
                    <h1 className='text-gray-500'>@{auth.findUser?.fullName.split(" ").join("_").toLowerCase()}</h1>
                </div>
                <div className='mt-2 space-y-3'>
                    <p>{auth.findUser?.bio}
                    </p>
                    <div className='py-1 flex space-x-5'>
                        {/*<div className='flex items-center text-gray-500'>
                            <PaletteIcon />
                            <p className='ml-2'>{auth.findUser?.website}</p>
                        </div>*/}
                        <div className='flex items-center text-blue-400'>
                            <OpenInNewIcon />
                            {auth.findUser?.website ? (
                            <a className='ml-2' 
                            href={auth.findUser.website} 
                            target="_blank" 
                            rel="noopener noreferrer">Visit Website</a>):(
                                <p className='m1-2'>            </p>
                            )}
                        </div>
                        <div className='flex items-center text-gray-500'>
                            <LocationOnIcon />
                            <p className='ml-2'>{auth.findUser?.location}</p>
                        </div>
                        <div className='flex items-center text-gray-500'>
                            <CalendarMonthIcon />
                            <p className='ml-2'>Joined July 2023</p>
                        </div>
                    </div>

                    <div className='flex items-center space-x-5'>
                        <div className='flex items-center space-x-1 font-semibold'>
                            <span>{auth.findUser?.following?.length}</span>
                            <span className='text-gray-500'>Following</span>

                        </div>
                        <div className='flex items-center space-x-1 font-semibold'>
                            <span>{auth.findUser?.followers?.length}</span>
                            <span className='text-gray-500'>Followers</span>

                        </div>
                    </div>

                </div>
            </section>

            <section className='py-5'>
                <Box sx={{ width: '100%', typography: 'body1' }}>
                    <TabContext value={tabValue}>
                        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                            <TabList onChange={handleTabChange} aria-label="lab API tabs example">
                                <Tab label="Chirps" value="1" />
                                <Tab label="Replies" value="2" />
                                <Tab label="Media" value="3" />
                                <Tab label="Likes" value="4" />
                            </TabList>
                        </Box>
                        {/*<TabPanel value="1">
                            {chirp.chirps.map((item)=><ChirpCard item = {item}/>)}
                        </TabPanel>*/}
                        <TabPanel value="1">
                            {chirp.usersChirps[id]?.map((item) => <ChirpCard item={item} />)}
                        </TabPanel>

                        <TabPanel value="2">User's replies</TabPanel>
                        <TabPanel value="3">Media</TabPanel>
                        <TabPanel value="4">
                        {chirp.likedChirps[id]?.map((item) => <ChirpCard item={item} />)}
                        </TabPanel>
                    </TabContext>
                </Box>
            </section>

            <section>
                <ProfileModal handleClose={handleClose} open={openProfileModal} />
            </section>
        </div>
    )
}

export default Profile